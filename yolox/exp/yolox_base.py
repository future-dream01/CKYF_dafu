...
class Exp(BaseExp):                 # 实验配置类
    ...
    def get_model(self):                # 获取训练模型
        from yolox.models import YOLOX, YOLOPAFPN, YOLOXHead ,YOLOGhostPAN,ShuffleV2PAFPN,YOLOFPN
        def init_yolo(M):
            for m in M.modules():
        ...
        if getattr(self, "model", None) is None:
            ...
            backbone = YOLOFPN(self.depth, self.width, in_channels=in_channels, act=self.act)
